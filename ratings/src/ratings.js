    ...
    if (process.env.DB_TYPE === 'mysql') {
     ...
     MongoClient.connect(url, (err, client) => {
      ...
      if (process.env.SERVICE_VERSION === 'v-faulty') {
        ...
        var random = crypto.randomBytes(4).readUInt32LE() / 0xffffffff; // returns [0,1]
        ...
        // in another half proceed as usual
        const crypto = require('crypto');
        var random2 = crypto.randomBytes(4).readUInt32LE() / 0xffffffff; // returns [0,1]
...
dispatcher.onGet('/health', function (req, res) {
    if (healthy) {
        res.writeHead(200, {'Content-type': 'application/json'})
        res.end(JSON.stringify({status: 'Ratings is healthy4'}))
    } else {
        res.writeHead(500, {'Content-type': 'application/json'})
